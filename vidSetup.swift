...
struct vidSetup: View {
    ...
    @State var folderName: String = "NewFolder"
    let size = 1024
    var body: some View {
        VStack {
            Text("Setup")
    ...
    func processVideo() {
            outputFolder = createFolder(atURL: outputFolder!.appending(path: "\(folderName)/"))
            ...
            Task {
                do {
                    ...
                    for i in stride(from: 0, to: durationInSeconds, by: timeInterval) {
                        ...
                        let nsImage = cropImageToSquare(image: nsImageT, size: NSSize(width: size, height: size))
                        ...
                        if let data = nsImage!.tiffRepresentation {
                            ...
                            totalFrames = Int(i / timeInterval)
